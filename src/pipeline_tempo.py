...
def extrair_dados(cidade, dias=7):
    """
    Extrai os dados de previsão do tempo da API VISUAL CROSSING para a cidade e período especificados.
    ...
    city_encoded = quote(cidade)# para lidar com espaços e caracteres especiais na cidade
...
def processar_dados(df):
    """
    Criando Colunas novas e selecionando apenas as necessárias.
...
def carregar_postgres(df, tabela="previsao_tempo"):
    ...
    # transforma df em lista de tuplas para inserção mais rápida
    registros = [
        (
            row["name"],
            row["datetime"],
    ...
    #Insere os valores usando UPSERT para evitar duplicação de registros
    query = f"""
        INSERT INTO {tabela} (
            name, datetime, tempmax, tempmin, temp,
            feelslike, precip, humidity, windspeed,
